...
import numpy as np
global alpha,mc,mb,mt,mw,mz,mu,muw,alphasz,z
alpha=1/137.056;
mc=1.27;
mb=4.8;
mt=164;
...
c8ef=eta**(14/23)*c80(xtw,yth)+0.8623*eta**(14/23)-0.9135*eta**0.4086+0.0873* eta**-0.4230-0.0571*eta**-0.8994+0.0209*eta**0.1456;
fg=N(0.29**2);
...
vmu(x_,y_)= alphas(mu)/(4*Pi)*(c1ef*r1+c2ef*r2+c7ef(xtw,yth)*r7+c8ef*r8 -16/3*c7ef(xtw,yth));
...
c41=e0(xtw)+ 2/3*Log(muw**2/mw**2)+U1**2*eh(yth);
w7sm =(-16*x**4-122*x**3+80*x**2-8*x)/(9*(x-1)**4)*PolyLog(2,1-1/x) +(6*x**4+46* x**3-28*x**2)/(3*(x-1)**5)*Log(x)**2+1/(81*(x-1)**5)*(-102*x**5-588*x**4-2262*x**3+3244*x**2-1364*x +208)*Log(x) +(1646* x**4+12205*x**3-10740*x**2+2509*x-436)/(486*(x-1)**4);
m7sm=1/(81*(x-1)**5)*(82*x**5+301*x**4+703*x**3-2197*x**2+1319*x-208-(162*x**4+1242*x**3-756*x**2)*Log(x));
t7sm=x/3*(1/(x-1)**5*(47*x**3-63*x**2+9*x+7-(18*x**3+30*x**2-24*x)*Log(x)) );
c71sm(x_)=w7sm+m7sm*Log(muw**2/mw**2)+t7sm*(Log(mt**2/muw**2)-4/3);
...
c71yy(x_)=w7yy+m7yy*Log(muw**2/mH**2)+t7sm/3*(Log(mt**2/muw**2)-4/3);
...
c71xy(x_)=w7xy+m7xy*Log(muw**2/mH**2)+t7xy*(Log(mt**2/muw**2)-4/3);
...
w8xy=1/3*((17*x**3-25*x**2+36*x)/(2*(x-1)**3)*PolyLog(2,1-1/x)-(17*x**2+19*x)/ ((x-1)**4)*Log(x)**2+(14*x**4-12*x**3+187*x**2+3*x )/(4*(x-1)**4)*Log(x)-(3*(29x**3-44*x**2+143*x))/( 8*(x-1)**3) );
m8xy=1/(6*(x-1)**4)*(-7*x**4+23*x**3-97*x**2+81*x+(34*x**2+38*x)*Log(x));
t8xy= 2*x*((-x**2-4*x+5+(4*x+2)*Log(x))/(x-1)**4 );
c81sm(x_)=w8sm+m8sm*Log(muw**2/mw**2)+t8sm*(Log(mt**2/muw**2)-4/3) ;
c81yy(x_)=w8yy+m8yy*Log(muw**2/mH**2)+t8sm/3*(Log(mt**2/muw**2)-4/3);
c81xy(x_)=w8xy+m8xy*Log(muw**2/mH**2)+t8xy*(Log(mt**2/muw**2)-4/3);
c81=c81sm(xtw)+U1**2*c81yy(yth)+U1*U2*c81xy(yth);
c11ef=15+6*Log(muw**2/mw**2) ;
c71eff=eta**(39/23)*c71+8/3*(eta**(37/23)-eta**(39/23))*c81+c80(xtw,yth)*(-(7164416/357075)*eta**(14/23)+297664/14283*eta**(16/23)+256868/14283*eta**(37/23)-6698884/357075*eta**(39/23))+37208/4761*(eta**(39/23)-eta**(16/23))*c70(xtw,yth)+(4661194/816831*eta**(14/23)-8516/2217*eta**(16/23)-1.9043*eta**0.4086-0.1008*eta**-0.4230+0.1216*eta**-0.8994+0.0183*eta**0.1456)eta*c41 -17.3023*eta**(14/23)+8.5027*eta**(16/23)+4.5508eta**(6/23)+0.7519eta**(-12/23)+2.0040*eta**0.4086+0.7476*eta**-0.4230-0.5385*eta**-0.8994+0.0914*eta**0.1456+(9.9372eta**(14/23)-7.4878*eta**(16/23)+1.2688eta**(6/23)-0.2925eta**(-12/23) -2.2923*eta**0.4086-0.1461*eta**-0.4230+0.1239*eta**-0.8994+0.0812*eta**0.1456 )*eta+(0.5784eta**(14/23)-0.3921*eta**(16/23)-0.1429eta**(6/23)+0.0476eta**(-12/23) -0.1275*eta**0.4086+0.0317*eta**-0.4230+0.0078*eta**-0.8994-0.0031*eta**0.1456)*eta*c11ef;
c7eft=c7ef(xtw,yth)+ alphas(mu)/(4*Pi)*c71eff;
bd=c7eft+vmu(xtw,yth);



Gt(t_)=If(t<4, -2 ArcTan(Sqrt(t/(4-t)))**2,-(Pi**2/2)+2*Log((Sqrt(t)+Sqrt(t-4))/2)**2-2*Pi*I*Log((Sqrt(t)+Sqrt(t-4))/2));
f220=(16*fg)/27*Integrate((1-fg*t)**2*Abs(Gt(t)/t+0.5)**2,{t,0,3.999});
f22p=(16*fg)/27*Integrate((1-fg*t)**2*Abs(Gt(t)/t+0.5)**2,{t,4,11.89});
f22=f220+f22p;
f270=-((8*fg**2)/9)Integrate((1-fg*t)(Gt(t)+0.5*t),{t,0,3.999});
f27p=-((8*fg**2)/9)Integrate((1-fg*t)(Gt(t)+0.5*t),{t,4,11.89});
f27=f270+f27p;
f11=f22/36;
f12=-(f22/3);
f17=-(f27/6);
...
A=alphas(mu)/ Pi*(c1ef**2*f11+c1ef*c2ef*f12+c1ef*c7ef(xtw,yth)*f17+c1ef*c8ef*f18+c2ef**2*f22+c2ef*c7ef(xtw,yth)*f27+c2ef*c8ef*f28+c7ef(xtw,yth)*c8ef*f78+c8ef**2*f88);
Delta=1/mb**2*(lambda1*0.5-4.5*lambda2)*c7ef(xtw,yth)**2+(-lambda2/(9*mc**2))(c7ef(xtw,yth)*(c2ef-c1ef/6));
brNLO(mH_,U1_,U2_)=N(Re(10.08/100*0.971*(6*alpha)/(Pi*fz*kz )*(Abs(bd)**2+ A+ Delta)));
N(brNLO(150,0,0))
'''
